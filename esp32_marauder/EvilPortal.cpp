...
void EvilPortal::setupServer() {
    ...
    server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
      ...
      #ifdef HAS_SCREEN
        this->sendToDisplay(F("客户端已连接到服务器"));
      #endif
    });
  #else
    server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
...
bool EvilPortal::setHtml() {
  ...
  if (!html_file) {
    #ifdef HAS_SCREEN
      this->sendToDisplay("找不到 /" + this->target_html_name);
      this->sendToDisplay(F("触摸退出..."));
    #endif
    Serial.println("找不到 /" + this->target_html_name + ". Use stopscan...");
    return false;
  }
  else {
    if (html_file.size() > MAX_HTML_SIZE) {
      #ifdef HAS_SCREEN
        this->sendToDisplay(F("HTML文件过大, 触摸退出..."));
      #endif
...
bool EvilPortal::setAP(LinkedList<ssid>* ssids, LinkedList<AccessPoint>* access_points) {
  ...
  if ((ssids->size() <= 0) && (temp_ap_name == "")) {
    ...
    if (!ap_config_file) {
      #ifdef HAS_SCREEN
        this->sendToDisplay(F("找不到 /ap.config.txt"));
        this->sendToDisplay(F("触摸退出..."));
      #endif
    ...
    // Config file good. Proceed
      ...
      if (ap_config_file.size() > MAX_AP_NAME_SIZE) {
        #ifdef HAS_SCREEN
          this->sendToDisplay(F("AP名称过长, 触摸退出..."));
        #endif
