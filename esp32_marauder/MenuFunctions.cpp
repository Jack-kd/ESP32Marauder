...
void MenuFunctions::buttonNotSelected(int b, int x) {
  ...
  uint16_t icon_color = (current_menu->list->get(x).icon == SETTINGS && current_menu->list->get(x).color == TFTLIGHTGREY) ? TFT_LIGHTGREY : color;
    ...
    drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + b * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(x).name.c_str(), TFT_BLACK, TFT_BLACK, color);
...
void MenuFunctions::buttonSelected(int b, int x) {
  ...
  uint16_t color = this->getColor(current_menu->list->get(x).color);
    ...
    if (current_menu->list->get(x).icon == SETTINGS && current_menu->list->get(x).color == TFTLIGHTGREY) {
      ...
      drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + b * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(x).name.c_str(), TFT_LIGHTGREY, TFT_BLACK, setting_color);
      display_obj.tft.drawXBitmap(0,
                                      KEY_Y + (b * (KEY_H + KEY_SPACING_Y)) - (ICON_H / 2),
                                      menu_icons[current_menu->list->get(x).icon],
                                      ICON_W,
      ...
      drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + b * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(x).name.c_str(), color, color, TFT_BLACK);
      if ((current_menu->list->get(x).name != text09) && (current_menu->list->get(x).icon != 255))
            display_obj.tft.drawXBitmap(0,
                                        KEY_Y + (b * (KEY_H + KEY_SPACING_Y)) - (ICON_H / 2),
                                        menu_icons[current_menu->list->get(x).icon],
...
bool MenuFunctions::isKeyPressed(char c)
{
  ...
  void MenuFunctions::buildUploadFileMenu() {
    if (sd_obj.supported) {
      ...
      this->addNodes(&uploadLogsMenu, "返回", TFTLIGHTGREY, 0, [this]() {
        this->changeMenu(uploadLogsMenu.parentMenu, true);
      });

      this->addNodes(&uploadLogsMenu, "删除Wardrive日志", TFTORANGE, 0, [this]() {
        this->changeMenu(&deleteAllMenu, true);
      });

      this->addNodes(&uploadLogsMenu, "全部上传", TFTGREEN, 0, [this]() {
...
void MenuFunctions::RunSetup()
{
  ...
  wifiScannerMenu.name = "扫描器";
  wifiAttackMenu.name = text_table1[21];
  wifiGeneralMenu.name = text_table1[22];
  saveFileMenu.name = "保存/加载文件";
  saveSSIDsMenu.name = "保存SSID";
  loadSSIDsMenu.name = "加载SSID";
  saveAPsMenu.name = "保存AP";
  loadAPsMenu.name = "加载AP";
  saveATsMenu.name = "保存Airtag";
  loadATsMenu.name = "加载Airtag";

  bluetoothSnifferMenu.name = text_table1[23];
  bluetoothAttackMenu.name = "蓝牙攻击";
  generateSSIDsMenu.name = text_table1[27];
  clearSSIDsMenu.name = text_table1[28];
  clearAPsMenu.name = text_table1[29];
  wifiAPMenu.name = "选择";
  wifiIPMenu.name = "活跃IP";
  apInfoMenu.name = "AP信息";
  setMacMenu.name = "设置MAC";
  genAPMacMenu.name = "生成AP MAC";
  wifiStationMenu.name = "选择终端";

  #ifdef HAS_DIRECT_UPLOAD
    uploadLogsMenu.name = "上传日志";
    uploadAllMenu.name = "全部上传?";
    deleteAllMenu.name = "全部删除?";
    actionMenu.name = "目标位置";
  #endif

  #ifdef HAS_GPS
    gpsMenu.name = "GPS"; 
    gpsInfoMenu.name = "GPS数据";
    //wardrivingMenu.name = "Wardriving";
  #endif  
  htmlMenu.name = "EP网页列表";
  miniKbMenu.name = "迷你键盘";

  #ifdef HAS_SD
    sdDeleteMenu.name = "删除SD文件";
  #endif

  selectProbeSSIDsMenu.name = "探测请求";
  evilPortalMenu.name = "邪恶门户";
  ssidsMenu.name = "SSID";

  #ifdef HAS_GPS
    gpsPOIMenu.name = "GPS兴趣点";
  #endif

  foxHuntMenu.name = "信号追踪";
  ...
  this->addNodes(&wifiMenu, "扫描器", TFTORANGE, SCANNERS, [this]() {
  ...
  this->addNodes(&wifiScannerMenu, "Ping扫描", TFTGREEN, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_PING_SCAN, TFT_CYAN);
  });
  #ifndef HAS_DUAL_BAND
    this->addNodes(&wifiScannerMenu, "ARP扫描", TFTCYAN, SCANNERS, [this]() {
      display_obj.clearScreen();
      this->drawStatusBar();
      wifi_scan_obj.StartScan(WIFI_ARP_SCAN, TFT_CYAN);
    });
  #endif
  this->addNodes(&wifiScannerMenu, "全端口扫描", TFTMAGENTA, BEACON_LIST, [this](){
    // Add the back button
  ...
  this->addNodes(&wifiScannerMenu, "SSH扫描", TFTORANGE, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_SSH, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "Telnet扫描", TFTRED, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_TELNET, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "SMTP扫描", TFTWHITE, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_SMTP, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "DNS扫描", TFTLIME, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_DNS, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "HTTP扫描", TFTSKYBLUE, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_HTTP, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "HTTPS扫描", TFTYELLOW, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_HTTPS, TFT_CYAN);
  });
  this->addNodes(&wifiScannerMenu, "RDP扫描", TFTPURPLE, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  this->addNodes(&wifiSnifferMenu, "数据包计数", TFTORANGE, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  #endif
  this->addNodes(&wifiSnifferMenu, "频道分析", TFTCYAN, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    this->renderGraphUI(WIFI_SCAN_CHAN_ANALYZER);
    wifi_scan_obj.StartScan(WIFI_SCAN_CHAN_ANALYZER, TFT_CYAN);
  });
  this->addNodes(&wifiSnifferMenu, "频道摘要", TFTORANGE, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  this->addNodes(&wifiSnifferMenu, "扫描AP/STA", TFTLIME, BEACON_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_AP_STA, 0x97e0);
  });
  /*this->addNodes(&wifiSnifferMenu, "信号追踪", TFTCYAN, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_SIG_STREN, TFT_CYAN);
  });*/
  this->addNodes(&wifiSnifferMenu, "信号追踪", TFTCYAN, SCANNERS, [this]() {
  ...
  this->addNodes(&wifiSnifferMenu, "MAC监控", TFTMAGENTA, SCANNERS, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_SCAN_DETECT_FOLLOW, TFT_MAGENTA);
  });
  this->addNodes(&wifiSnifferMenu, "SAE提交", TFTLIME, EAPOL, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  this->addNodes(&wifiAttackMenu, "邪恶门户", TFTORANGE, BEACON_SNIFF, [this]() {
  ...
  this->addNodes(&wifiAttackMenu, "Karma攻击", TFTORANGE, KEYBOARD_ICO, [this](){
    // Add the back button
  ...
  this->addNodes(&wifiAttackMenu, "恶意消息", TFTRED, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_BAD_MSG, TFT_RED);
  });
  this->addNodes(&wifiAttackMenu, "定向恶意消息", TFTYELLOW, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_BAD_MSG_TARGETED, TFT_YELLOW);
  });
  this->addNodes(&wifiAttackMenu, "关联休眠", TFTRED, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_SLEEP, TFT_RED);
  });
  this->addNodes(&wifiAttackMenu, "定向关联休眠", TFTMAGENTA, DEAUTH_SNIFF, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_SLEEP_TARGETED, TFT_MAGENTA);
  });
  this->addNodes(&wifiAttackMenu, "SAE提交洪水", TFTLIME, EAPOL, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_SAE_COMMIT, TFT_GREEN);
  });
  this->addNodes(&wifiAttackMenu, "频道切换", TFTORANGE, BEACON_LIST, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
    wifi_scan_obj.StartScan(WIFI_ATTACK_CSA, TFT_GREEN);
  });
  this->addNodes(&wifiAttackMenu, "静默时间", TFTRED, BEACON_LIST, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  this->addNodes(&evilPortalMenu, "接入点", TFTGREEN, BEACON_SNIFF, [this]() {
    this->changeMenu(&wifiAPMenu, true);
  });
  this->addNodes(&evilPortalMenu, "用户SSID", TFTCYAN, PROBE_SNIFF, [this]() {
  ...
  clearSSIDsMenu.parentMenu = &wifiGeneralMenu;
    ...
    this->addNodes(&wifiGeneralMenu, text_table1[1], TFTNAVY, KEYBOARD_ICO, [this](){
      ...
      while (keep_going) {
        display_obj.clearScreen(); 
        if (keyboardInput(ssidBuf, sizeof(ssidBuf), "输入SSID")) {
          if (ssidBuf[0] != 0)
  ...
  //#else // Mini EP HTML select
    this->addNodes(&wifiGeneralMenu, "选择EP网页文件", TFTCYAN, KEYBOARD_ICO, [this](){
      // Add the back button
      htmlMenu.list->clear();
    ...
    // Select APs on Mini
    this->addNodes(&wifiGeneralMenu, "选择AP", TFTNAVY, KEYBOARD_ICO, [this](){
      ...
      this->addNodes(&wifiAPMenu, "全选", TFTGREEN, 255, [this](){
    ...
    this->addNodes(&wifiGeneralMenu, "查看AP信息", TFTCYAN, KEYBOARD_ICO, [this](){
      wifiAPMenu.parentMenu = &wifiGeneralMenu;
    ...
    this->addNodes(&wifiGeneralMenu, "选择终端", TFTCYAN, KEYBOARD_ICO, [this](){
      wifiAPMenu.parentMenu = &wifiGeneralMenu;
      ...
      for (int i = 0; i < menu_limit; i++) {
        ...
        this->addNodes(&wifiAPMenu, access_points->get(i).essid.c_str(), TFTCYAN, 255, [this, i](){
          ...
          this->addNodes(&wifiStationMenu, "全选", TFTGREEN, 255, [this, i](){
    ...
    this->addNodes(&wifiGeneralMenu, "加入WiFi", TFTWHITE, KEYBOARD_ICO, [this](){
      ...
      for (int i = 0; i < access_points->size(); i++) {
        ...
        this->addNodes(&wifiAPMenu, access_points->get(i).essid.c_str(), TFTCYAN, 255, [this, i](){
          ...
          // Join WiFi using touch screen keyboard
            ...
            if (keyboardInput(passwordBuf, sizeof(passwordBuf), "输入密码")) {
    ...
    this->addNodes(&wifiGeneralMenu, "加入已保存WiFi", TFTWHITE, KEYBOARD_ICO, [this](){
    ...
    this->addNodes(&wifiGeneralMenu, "启动AP", TFTGREEN, KEYBOARD_ICO, [this](){
  ...
  this->addNodes(&wifiGeneralMenu, "设置MAC", TFTLIGHTGREY, 0, [this]() {
  ...
  this->addNodes(&wifiGeneralMenu, "Shutdown WiFi", TFTRED, 0, [this]() {
  ...
  #ifdef HAS_DIRECT_UPLOAD
    this->addNodes(&wifiGeneralMenu, "上传Wardrive日志", TFTGREEN, 0, [this]() {
      display_obj.clearScreen();
      display_obj.tft.setTextWrap(false);
      display_obj.tft.setCursor(0, SCREEN_HEIGHT / 3);
      display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
      display_obj.tft.println("加载中...");
    ...
    this->addNodes(&uploadAllMenu, "WiGLE", TFTLIGHTGREY, 0, [this]() {
      ...
      if ((ssid == "") && (pw == "")) {
        ...
        display_obj.tft.setCursor(0, SCREEN_HEIGHT / 3);
        display_obj.tft.println("WiFi凭据为空");
        display_obj.tft.println("返回中...");
        display_obj.tft.setTextWrap(false);
      }
      else {
        display_obj.clearScreen();
        display_obj.showCenterText(String("正在连接 " + ssid).c_str(), TFT_HEIGHT / 2, true);
        if (!wifi_scan_obj.joinWiFi(ssid, pw, false)) {
          display_obj.clearScreen();
          display_obj.tft.setTextWrap(true);
          display_obj.tft.setCursor(0, SCREEN_HEIGHT / 3);
          display_obj.tft.println("无法连接WiFi");
          display_obj.tft.println("返回中...");
          display_obj.tft.setTextWrap(false);
        }
        else {
            ...
            if (sd_obj.sd_files->get(i).startsWith("wardrive_") || sd_obj.sd_files->get(i).startsWith("wigle-")) {
              if (!sd_obj.sd_files->get(i).endsWith(".wigle") && !sd_obj.sd_files->get(i).endsWith(".wdg") && !sd_obj.sd_files->get(i).endsWith(".gpx")) {
                ...
                if (wifi_scan_obj.uploadFile("/" + sd_obj.sd_files->get(i), true, WIGLE_UPLOAD)) {
                  display_obj.clearScreen();
                  display_obj.showCenterText("WiGLE上传成功", TFT_HEIGHT / 2);
                } else {
                  display_obj.clearScreen();
                  display_obj.showCenterText("WiGLE上传失败", TFT_HEIGHT / 2);
                }
              }
            }
          }
            ...
            if (sd_obj.sd_files->get(i).startsWith("wardrive_") || sd_obj.sd_files->get(i).startsWith("wigle-")) {
              if (!sd_obj.sd_files->get(i).endsWith(".wigle") && !sd_obj.sd_files->get(i).endsWith(".wdg") && !sd_obj.sd_files->get(i).endsWith(".gpx")) {
                ...
                if (wifi_scan_obj.uploadFile("/" + sd_obj.sd_files->get(i), true, WDG_UPLOAD)) {
                  display_obj.clearScreen();
                  display_obj.showCenterText("WDG上传成功", TFT_HEIGHT / 2);
                } else {
                  display_obj.clearScreen();
                  display_obj.showCenterText("WDG上传失败", TFT_HEIGHT / 2);
                }
              }
            }
          }
    ...
    this->addNodes(&deleteAllMenu, "Yes", TFTRED, 0, [this]() {
      ...
      display_obj.showCenterText("正在删除日志...", TFT_HEIGHT / 2, true);
      ...
      display_obj.showCenterText("日志已删除", TFT_HEIGHT / 2, true);
      ...
      if ((ssid == "") && (pw == "")) {
        ...
        if (!wifi_scan_obj.joinWiFi(ssid, pw, false)) {
          ...
          if (wifi_scan_obj.uploadFile("/" + sd_obj.selected_file_name, true, WIGLE_UPLOAD)) {
            display_obj.clearScreen();
            display_obj.showCenterText("WiGLE上传成功", TFT_HEIGHT / 2, true);
          } else {
            display_obj.clearScreen();
            display_obj.showCenterText("WiGLE上传失败", TFT_HEIGHT / 2, true);
          }
      ...
      if ((ssid == "") && (pw == "")) {
        ...
        if (!wifi_scan_obj.joinWiFi(ssid, pw, false)) {
          ...
          if (wifi_scan_obj.uploadFile("/" + sd_obj.selected_file_name, true, WDG_UPLOAD)) {
            display_obj.clearScreen();
            display_obj.showCenterText("WDG上传成功", TFT_HEIGHT / 2, true);
          } else {
            display_obj.clearScreen();
            display_obj.showCenterText("WDG上传失败", TFT_HEIGHT / 2, true);
          }
      ...
      if ((ssid == "") && (pw == "")) {
        ...
        if (!wifi_scan_obj.joinWiFi(ssid, pw, false)) {
          ...
          if (wifi_scan_obj.uploadFile("/" + sd_obj.selected_file_name, true, BOTH_UPLOAD)) {
            display_obj.clearScreen();
            display_obj.showCenterText("上传成功", TFT_HEIGHT / 2, true);
          } else {
            display_obj.clearScreen();
            display_obj.showCenterText("上传失败", TFT_HEIGHT / 2, true);
          }
  ...
  this->addNodes(&setMacMenu, "生成AP MAC", TFTLIME, 0, [this]() {
  ...
  this->addNodes(&bluetoothMenu, "蓝牙攻击", TFTRED, ATTACKS, [this]() {
  ...
  this->addNodes(&bluetoothSnifferMenu, "蓝牙分析器", TFTCYAN, PACKET_MONITOR, [this]() {
    display_obj.clearScreen();
    this->drawStatusBar();
  ...
  this->addNodes(&bluetoothSnifferMenu, "信号追踪", TFTCYAN, SCANNERS, [this]() {
    foxHuntMenu.list->clear();
  ...
  this->addNodes(&deviceMenu, text09, TFTLIGHTGREY, 0, [this]() {
    ...
    if (sd_obj.supported) {
      ...
      this->addNodes(&deviceMenu, "固件更新", TFTORANGE, SD_UPDATE, [this]() {
        display_obj.clearScreen();
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setCursor(0, SCREEN_HEIGHT / 3);
        display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
  ...
  this->addNodes(&deviceMenu, "保存/加载文件", TFTCYAN, SD_UPDATE, [this]() {
    this->changeMenu(&saveFileMenu, true);
  });

  #ifndef HAS_MINI_SCREEN
    this->addNodes(&deviceMenu, "亮度", TFTYELLOW, BRIGHTNESS, [this]() {
      this->brightnessMode();
    });
  #endif
  ...
  this->addNodes(&deviceMenu, text08, TFTBLUE, SETTINGS, [this]() {
    ...
    if (sd_obj.supported) {
      ...
      this->addNodes(&deviceMenu, "删除SD文件", TFTCYAN, SD_UPDATE, [this]() {
        display_obj.clearScreen();
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setCursor(0, SCREEN_HEIGHT / 3);
        display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
  ...
  this->addNodes(&saveFileMenu, "保存SSID", TFTCYAN, SD_UPDATE, [this]() {
    this->changeMenu(&saveSSIDsMenu, true);
    wifi_scan_obj.RunSaveSSIDList(true);
  });
  this->addNodes(&saveFileMenu, "加载SSID", TFTSKYBLUE, SD_UPDATE, [this]() {
    this->changeMenu(&loadSSIDsMenu, true);
    wifi_scan_obj.RunLoadSSIDList();
  });
  this->addNodes(&saveFileMenu, "保存AP", TFTNAVY, SD_UPDATE, [this]() {
    this->changeMenu(&saveAPsMenu, true);
    wifi_scan_obj.RunSaveAPList();
  });
  this->addNodes(&saveFileMenu, "加载AP", TFTBLUE, SD_UPDATE, [this]() {
    this->changeMenu(&loadAPsMenu, true);
    wifi_scan_obj.RunLoadAPList();
  });
  this->addNodes(&saveFileMenu, "保存Airtag", TFTWHITE, SD_UPDATE, [this]() {
    this->changeMenu(&saveAPsMenu, true);
    wifi_scan_obj.RunSaveATList();
  });
  this->addNodes(&saveFileMenu, "加载Airtag", TFTWHITE, SD_UPDATE, [this]() {
    this->changeMenu(&loadAPsMenu, true);
  ...
  // GPS Menu
    ...
    if (gps_obj.getGpsModuleStatus()) {
      ...
      this->addNodes(&gpsMenu, "GPS数据", TFTRED, GPS_MENU, [this]() {
        wifi_scan_obj.currentScanMode = WIFI_SCAN_GPS_DATA;
      ...
      this->addNodes(&gpsMenu, "GPS兴趣点", TFTCYAN, GPS_MENU, [this]() {
      ...
      this->addNodes(&gpsPOIMenu, "Mark POI", TFTCYAN, GPS_MENU, [this]() {
        ...
        if (wifi_scan_obj.RunGPSInfo(true, false, true))
          display_obj.showCenterText("POI已记录", TFT_HEIGHT / 2);
        else
          display_obj.showCenterText("POI记录失败", TFT_HEIGHT / 2);
        wifi_scan_obj.currentScanMode = WIFI_SCAN_OFF;
        delay(2000);
        this->changeMenu(&gpsPOIMenu, true);
      });
      ...
      //#if !(defined(MARAUDER_V6) || defined(MARAUDER_V6_1) || defined(MARAUDER_CYD_MICRO))
        while(true) {
          ...
          if (pressed) {
            ...
            #ifdef HAS_MINI_KB
              #if !defined(MARAUDER_CARDPUTER) && !defined(MARAUDER_CARDPUTER_ADV)
              display_obj.tft.println("上/下 - 删/加字符");
              display_obj.tft.println("左/右 - 前/后字符");
              #endif
              if (!do_pass) {
                #if defined(MARAUDER_CARDPUTER) || defined(MARAUDER_CARDPUTER_ADV)
                  display_obj.tft.println("确认 - 保存");
                  display_obj.tft.println("Esc - 退出");
                #else
                  display_obj.tft.println("C - 保存");
                  display_obj.tft.println("C(长按) - 退出");
                #endif
              }
              else {
                #if defined(MARAUDER_CARDPUTER) || defined(MARAUDER_CARDPUTER_ADV)
                  display_obj.tft.println("确认 - 输入");
                #else
                  display_obj.tft.println("C(长按) - 输入");
                #endif
              }
            #endif

            #ifdef HAS_TOUCH
              display_obj.tft.println("上/下 - 前/后字符");
              display_obj.tft.println("C - 添加字符");
              display_obj.tft.println("C(长按) - 删除字符");
              display_obj.tft.println("U(长按) - 输入");
            #endif
            pressed = false;
          }
        }
...
void MenuFunctions::renderGraphUI(uint8_t scan_mode) {
  ...
  if (scan_mode == WIFI_SCAN_CHAN_ANALYZER)
    drawCentreStringChinese(display_obj.tft, (String("帧/") + String(BANNER_TIME) + "ms").c_str(), SCREEN_WIDTH / 2, SCREEN_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2));
  else if (scan_mode == BT_SCAN_ANALYZER)
    drawCentreStringChinese(display_obj.tft, (String("BLE信标/") + String(BANNER_TIME) + "ms").c_str(), SCREEN_WIDTH / 2, SCREEN_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2));
  display_obj.tft.drawLine(0, SCREEN_HEIGHT - GRAPH_VERT_LIM - 1, SCREEN_WIDTH, SCREEN_HEIGHT - GRAPH_VERT_LIM - 1, TFT_WHITE);
  int32_t labelY = SCREEN_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 8);
  display_obj.tft.setTextSize(1);
  display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  display_obj.tft.setCursor(0, labelY);
  printChinese(display_obj.tft, "最大");
  labelY += chinese_font.yAdvance;
  display_obj.tft.setTextColor(TFT_ORANGE, TFT_BLACK);
  display_obj.tft.setCursor(0, labelY);
  printChinese(display_obj.tft, "平均");
  labelY += chinese_font.yAdvance;
  if (scan_mode != BT_SCAN_ANALYZER) {
    display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
    display_obj.tft.setCursor(0, labelY);
    printChinese(display_obj.tft, "频道标记");
  }
}
    ...
    for (uint16_t i = start_index; i < min(start_index + BUTTON_SCREEN_LIMIT, current_menu->list->size()); i++)
    {
      ...
      uint16_t color = this->getColor(current_menu->list->get(i).color);
        ...
        if (is_setting_node && current_menu->selected == i) {
          ...
          drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + (i - start_index) * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(i).name.c_str(), TFT_LIGHTGREY, TFT_BLACK, setting_color);
          display_obj.tft.drawXBitmap(0,
          ...
          drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + (i - start_index) * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(i).name.c_str(), color, color, TFT_BLACK);
          ...
          drawChineseButton(display_obj.tft, KEY_X - KEY_W/2, KEY_Y + (i - start_index) * (KEY_H + KEY_SPACING_Y) - KEY_H/2, KEY_W, KEY_H, current_menu->list->get(i).name.c_str(), TFT_BLACK, TFT_BLACK, is_setting_node ? TFT_LIGHTGREY : color);
          if ((current_menu->list->get(i).name != text09) && (current_menu->list->get(i).icon != 255))
            display_obj.tft.drawXBitmap(0,
                                        KEY_Y + (i - start_index) * (KEY_H + KEY_SPACING_Y) - (ICON_H / 2),
                                        menu_icons[current_menu->list->get(i).icon],
...
// ============================================================
  ...
  void MenuFunctions::brightnessMode() {
    ...
    display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
    drawCentreStringChinese(display_obj.tft, "亮度", TFT_WIDTH/2, 30);

    display_obj.tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
    drawCentreStringChinese(display_obj.tft, "点按上方=更亮", TFT_WIDTH/2, 10);
    drawCentreStringChinese(display_obj.tft, "点按下方=更暗", TFT_WIDTH/2, TFT_HEIGHT - 20);
    display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
