...
void WiFiScan::showNetworkInfo() {
  ...
  Serial.println(WiFi.macAddress());
    ...
    int32_t infoY = display_obj.tft.getCursorY();
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, "已连接!");
    infoY += chinese_font.yAdvance;
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, ("IP地址: " + this->ip_addr.toString()).c_str());
    infoY += chinese_font.yAdvance;
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, ("网关: " + this->gateway.toString()).c_str());
    infoY += chinese_font.yAdvance;
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, ("子网掩码: " + this->subnet.toString()).c_str());
    infoY += chinese_font.yAdvance;
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, ("MAC: " + WiFi.macAddress()).c_str());
    infoY += chinese_font.yAdvance;
    display_obj.tft.setCursor(0, infoY);
    printChinese(display_obj.tft, "返回中...");
    delay(2000);
  #endif
}
...
bool WiFiScan::joinWiFi(String ssid, String password, bool gui) {
  ...
  WiFi.begin(ssid.c_str(), password.c_str());
    ...
    if (gui) {
      ...
      display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
      printChinese(display_obj.tft, "正在连接");
...
bool WiFiScan::startWiFi(String ssid, String password, bool gui) {
  ...
  if (password != "")
    ...
    if (gui) {
      ...
      display_obj.tft.setTextSize(1);
      display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
      printChinese(display_obj.tft, "正在启动");
      display_obj.tft.setTextWrap(true, false);
    }
  #endif
...
void WiFiScan::StartScan(uint8_t scan_mode, uint16_t color) {  
  ...
  if (scan_mode == WIFI_SCAN_PROBE)
   ...
   this->startWiFiAttacks(scan_mode, color, "Quiet Attack");
    ...
    this->startWiFiAttacks(scan_mode, color, "恶意消息");
...
void WiFiScan::displayTargetFilter() {
    ...
    if (this->filterActive()) {
      ...
      display_obj.tft.setTextSize(1);
      display_obj.showCenterText("正在发送...", (STATUS_BAR_WIDTH * 3) + 1, true);
      ...
      display_obj.showCenterText("目标网络", (STATUS_BAR_WIDTH * 3) + (CHAR_WIDTH) + 1);
      display_obj.tft.setTextSize(1);
      display_obj.tft.println();
      display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
      for (int i = 0; i < access_points->size(); i++) {
      ...
      display_obj.showCenterText("未选择网络", (STATUS_BAR_WIDTH * 3) + (CHAR_WIDTH));
    }
  #endif
}
  ...
  // Common wifi attack configurations
    ...
    this->setupScanDisplayArea(TFT_BLACK, color);
      ...
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
            drawCentreStringChinese(display_obj.tft, title_string, TFT_WIDTH / 2, 16);
    #endif
    #ifdef HAS_ILI9341
...
void WiFiScan::RunPingScan(uint8_t scan_mode, uint16_t color) {
  ...
  /*#ifdef HAS_FLIPPER_LED
    ...
    this->setupScanDisplayArea(TFT_BLACK, color);
      ...
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      if (scan_mode == WIFI_PING_SCAN)
        drawCentreStringChinese(display_obj.tft, "Ping扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_ARP_SCAN)
        drawCentreStringChinese(display_obj.tft, "ARP扫描", TFT_WIDTH / 2, 16);
    #endif
  ...
  /*#ifdef HAS_FLIPPER_LED
    ...
    this->setupScanDisplayArea(TFT_BLACK, color);
      ...
      if (scan_mode == WIFI_PORT_SCAN_ALL)
        drawCentreStringChinese(display_obj.tft, "全端口扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_SSH)
        drawCentreStringChinese(display_obj.tft, "SSH扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_TELNET)
        drawCentreStringChinese(display_obj.tft, "Telnet扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_SMTP)
        drawCentreStringChinese(display_obj.tft, "SMTP扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_DNS)
        drawCentreStringChinese(display_obj.tft, "DNS扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_HTTP)
        drawCentreStringChinese(display_obj.tft, "HTTP扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_HTTPS)
        drawCentreStringChinese(display_obj.tft, "HTTPS扫描", TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_RDP)
        drawCentreStringChinese(display_obj.tft, "RDP扫描", TFT_WIDTH / 2, 16);
    #endif
    this->prepareScanStage(TFT_RED, TFT_BLACK);
  #endif
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
            drawCentreStringChinese(display_obj.tft, " 邪恶门户 ", TFT_WIDTH / 2, 16);
    #endif
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    if (scan_mode != WIFI_SCAN_AP_STA)
      ...
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      if (scan_mode != WIFI_SCAN_AP_STA)
        drawCentreStringChinese(display_obj.tft, text_table4[44], TFT_WIDTH / 2, 16);
      else
        drawCentreStringChinese(display_obj.tft, "扫描AP/STA", TFT_WIDTH / 2, 16);
    #endif
  ...
  if (scan_mode == WIFI_PACKET_MONITOR)
    ...
    if ((scan_mode != WIFI_SCAN_PACKET_RATE) &&
      ...
      this->setupScanDisplayArea(TFT_WHITE, color);
        ...
        display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
        if (scan_mode == WIFI_PACKET_MONITOR)
          drawCentreStringChinese(display_obj.tft, text_table1[45], TFT_WIDTH / 2, 16);
        else if (scan_mode == WIFI_SCAN_CHAN_ANALYZER) {
          display_obj.tft.setTextColor(TFT_BLACK, color);
          drawCentreStringChinese(display_obj.tft, "频道分析", TFT_WIDTH / 2, 16);
        }
        else if (scan_mode == WIFI_SCAN_CHAN_ACT) {
          display_obj.tft.setTextColor(TFT_BLACK, color);
          drawCentreStringChinese(display_obj.tft, "频道摘要", TFT_WIDTH / 2, 16);
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color) {
  ...
  /*#ifdef HAS_ILI9341
    ...
    startPcap("eapol");
      ...
      #ifdef HAS_FULL_SCREEN
        display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
        drawCentreStringChinese(display_obj.tft, "EAPOL嗅探", TFT_WIDTH / 2, 16);
      #endif
      display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
      display_obj.tftDrawChannelScaleButtons(set_channel, false);
      display_obj.tftDrawExitScaleButtons(false);
...
void WiFiScan::RunPineScan(uint8_t scan_mode, uint16_t color) {
  ...
  /*#ifdef HAS_FLIPPER_LED
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      drawCentreStringChinese(display_obj.tft, text_table4[48], TFT_WIDTH / 2, 16);
    #endif
    this->prepareScanStage(TFT_RED, TFT_BLACK);
  #endif
...
void WiFiScan::RunMultiSSIDScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      drawCentreStringChinese(display_obj.tft, text_table4[49], TFT_WIDTH / 2, 16);
    #endif
...
void WiFiScan::RunPwnScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      drawCentreStringChinese(display_obj.tft, text_table4[37], TFT_WIDTH / 2, 16);
    #endif
    this->prepareScanStage(TFT_GREEN, TFT_BLACK);
  #endif
...
void WiFiScan::displayWardriveStats() {
      ...
      display_obj.tft.fillRect(0,
        ...
        display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
        display_obj.tft.println("蓝牙: " + (String)this->bt_frames);
        display_obj.tft.setTextSize(2);
        display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
        display_obj.tft.println("集群: " + (String)this->flock_devices + "\n");
      #else
        display_obj.tft.setTextSize(2);
        display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
        display_obj.tft.println("WiFi:" + (String)this->beacon_frames);
...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    this->setupScanDisplayArea(TFT_WHITE, color);
      ...
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      if (scan_mode == WIFI_SCAN_AP)
        drawCentreStringChinese(display_obj.tft, text_table4[38], TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_WAR_DRIVE) {
...
void WiFiScan::RunRawScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    this->setupScanDisplayArea(TFT_WHITE, color);
      ...
      if (scan_mode != WIFI_SCAN_SIG_STREN) {
        display_obj.tft.setTextColor(TFT_BLACK, color);
        drawCentreStringChinese(display_obj.tft, text_table1[58], TFT_WIDTH / 2, 16);
      }
      else {
        display_obj.tft.setTextColor(TFT_BLACK, color);
        drawCentreStringChinese(display_obj.tft, "信号追踪", TFT_WIDTH / 2, 16);
      }
    #endif
    display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
...
void WiFiScan::RunDeauthScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      drawCentreStringChinese(display_obj.tft, text_table4[39], TFT_WIDTH / 2, 16);
    #endif
    display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
...
void WiFiScan::RunSAEScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    #ifdef HAS_FULL_SCREEN
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      drawCentreStringChinese(display_obj.tft, "SAE提交", TFT_WIDTH / 2, 16);
    #endif
    this->prepareScanStage(TFT_GREEN, TFT_BLACK);
  #endif
...
void WiFiScan::RunProbeScan(uint8_t scan_mode, uint16_t color) {
  ...
  this->setLEDMode(MODE_SNIFF);
    ...
    this->setupScanDisplayArea(TFT_BLACK, color);
      ...
      display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
      if (scan_mode == WIFI_SCAN_PROBE)
        drawCentreStringChinese(display_obj.tft, text_table4[40], TFT_WIDTH / 2, 16);
      else if (scan_mode == WIFI_SCAN_DETECT_FOLLOW) 
        drawCentreStringChinese(display_obj.tft, "MAC监控", TFT_WIDTH / 2, 16);
      else {
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color) {
        ...
        (scan_mode == BT_SCAN_SIMPLE_TWO))
    {
        ...
        this->setupScanDisplayArea(TFT_BLACK, color);
          ...
          display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
          if (scan_mode == BT_SCAN_ALL)
            drawCentreStringChinese(display_obj.tft, text_table4[41], TFT_WIDTH / 2, 16);
      ...
      if (scan_mode == BT_SCAN_ALL) {
        ...
        display_obj.tft.setTextColor(TFT_BLACK, color);
        display_obj.tft.fillRect(0,16,TFT_WIDTH,16, color);
        drawCentreStringChinese(display_obj.tft, text_table4[42], TFT_WIDTH / 2, 16);
...
void WiFiScan::displayAnalyzerString(String str) {
    ...
    display_obj.tft.fillRect(0, 
                            ...
                            CHAR_WIDTH + 2, 
                            TFT_BLACK);
    //display_obj.tft.drawCentreString("帧/" + (String)BANNER_TIME + "ms", TFT_WIDTH / 2, TFT_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2), 1);
...
uint16_t WiFiScan::rssiToColor(int8_t rssi) {
  ...
  bool WiFiScan::uploadFile(String filePath, bool retry, uint8_t upload_type) {
    #ifdef HAS_SCREEN
    display_obj.clearScreen();
    display_obj.showCenterText(String("正在上传 " + filePath).c_str(), TFT_HEIGHT / 2, true);
    #endif
    delay(1000);
  ...
  bool WiFiScan::wdgwarsUpload(String filePath) {
    ...
    #ifdef HAS_SCREEN
    display_obj.clearScreen();
    display_obj.showCenterText("WDG上传中...", TFT_HEIGHT / 2, true);
    #endif
    delay(100);

    if (!SD.exists(filePath)) {
      #ifdef HAS_SCREEN
      display_obj.showCenterText(String(filePath + " 未找到").c_str(), TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    if (apiKey.isEmpty()) {
      #ifdef HAS_SCREEN
      display_obj.clearScreen();
      display_obj.showCenterText("无WDG API密钥", TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    if (!fileToUpload) {
      #ifdef HAS_SCREEN
      display_obj.clearScreen();
      display_obj.showCenterText("无法打开文件", TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    if (!client->connect("wdgwars.pl", 443)) {
      ...
      #ifdef HAS_SCREEN
      display_obj.clearScreen();
      display_obj.showCenterText("WDG连接失败", TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    #ifdef HAS_SCREEN
      display_obj.showCenterText("等待响应中...", (TFT_HEIGHT / 3) * 2, true);
    #endif
    ...
    #ifdef HAS_SCREEN
    display_obj.clearScreen();
    display_obj.showCenterText(ok ? "WDG上传成功" : "WDG上传失败", TFT_HEIGHT / 2, true);
    #endif
  ...
  bool WiFiScan::wigleUpload(String filePath) {
    ...
    #ifdef HAS_SCREEN
    display_obj.clearScreen();
    display_obj.showCenterText("Wigle上传中...", TFT_HEIGHT / 2, true);
    #endif
    ...
    if (username.isEmpty() || token.isEmpty()) {
      ...
      #ifdef HAS_SCREEN
      display_obj.clearScreen();
      display_obj.showCenterText("无Wigle凭据", TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    if (!client->connect("api.wigle.net", 443)) {
      ...
      #ifdef HAS_SCREEN
      display_obj.clearScreen();
      display_obj.showCenterText("无法连接", TFT_HEIGHT / 2, true);
      delay(2000);
      #endif
    ...
    #ifdef HAS_SCREEN
    display_obj.clearScreen();
    display_obj.showCenterText(ok ? "WIGLE上传成功" : "WIGLE上传失败", TFT_HEIGHT / 2, true);
    #endif
    ...
    if (currentTime - initTime >= this->channel_hop_delay * 500) {
      ...
      this->channelHop(true);
        ...
        display_obj.tft.fillRect(0,
          ...
          if (this->filterActive()) {
            display_obj.tft.setTextColor(TFT_RED, TFT_BLACK);
            display_obj.showCenterText("（已过滤）", (STATUS_BAR_WIDTH * 1) + CHAR_WIDTH + EXT_BUTTON_WIDTH);
          }
        #endif
        display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH + EXT_BUTTON_WIDTH);
        display_obj.tft.setTextSize(1);
