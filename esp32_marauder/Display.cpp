...
void Display::tftDrawColorKey()
{
  ...
  tft.fillRect(14, 0, 15, 8, TFT_GREEN); tft.setCursor(30, 0); printChinese(tft, " - 信标"); 
  tft.fillRect(14, 8, 15, 8, TFT_RED); tft.setCursor(30, 8); printChinese(tft, " - 解除认证");
  tft.fillRect(14, 16, 15, 8, TFT_BLUE); tft.setCursor(30, 16); printChinese(tft, " - 探测");
}

void Display::tftDrawXScaleButtons(byte x_scale) {
  tft.drawFastVLine(234, 0, 20, TFT_WHITE);
  tft.setCursor(208, 21); tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.setTextSize(1); printChinese(tft, "X缩放:"); tft.print(x_scale);
...
void Display::tftDrawYScaleButtons(byte y_scale)
{
  ...
  tft.setCursor(265, 21); tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.setTextSize(1); printChinese(tft, "Y缩放:"); tft.print(y_scale);
...
void Display::touchToExit()
{
  ...
  drawCentreStringChinese(tft, text11, TFT_WIDTH / 2, 32);
}
...
void Display::processAndPrintString(TFT_eSPI& _tft, const String& originalString) {
  // Define colors
